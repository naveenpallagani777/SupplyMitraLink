        ...
        <div className="px-4 sm:px-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
              <h2 className="text-gray-500 text-sm font-medium">{t('supplier.totalWarnings')}</h2>
              <p className="text-2xl font-bold text-gray-900">{warnings.length}</p>
            </div>
            <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
              <h2 className="text-gray-500 text-sm font-medium">{t('common.pending')}</h2>
              <p className="text-2xl font-bold text-yellow-600">{pendingCount}</p>
            </div>
            <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
              <h2 className="text-gray-500 text-sm font-medium">{t('common.resolved')}</h2>
                  ...
                  {filteredWarnings.map((warning) => (
                    <tr key={warning.id} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getSeverityColor(warning.severity)}`}>
                          {warning.severity}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(warning.status)}`}>
                          {warning.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
          ...
          {filteredWarnings.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">{t('supplier.noWarningsFound')}</h3>
              <p className="text-gray-500">{t('supplier.noWarningsMatching')}</p>
            </div>
          )}
        </div>
      </main>
