...
const Feedback = () => {
  ...
  const supplierRatings = React.useMemo(() => {
    ...
    return Array.from(supplierMap.values()).map(supplier => {
      const avgRating = supplier.ratings.reduce((sum, rating) => sum + rating, 0) / supplier.ratings.length;
      ...
      <main className="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        {supplierRatings.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">There are no supplier ratings to display at the moment.</p>
          </div>
        ) : (
            ...
            {supplierRatings.map((supplier) => (
              <div key={supplier.id} className="bg-white rounded-lg shadow p-6 flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="flex items-center mt-1">
                    ...
                    <span className="ml-2 text-sm text-gray-600">
                      {supplier.averageRating.toFixed(1)} ({supplier.totalReviews} {t('vendor.reviews')})
                    </span>
                  </div>
                ...
                <div className="mt-6 md:mt-0 md:ml-8 flex-shrink-0 flex items-center">
                  <Link
                    to={`/vendors/${encodeURIComponent(supplier.supplierName)}/public`}
