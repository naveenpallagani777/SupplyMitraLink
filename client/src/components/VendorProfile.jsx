...
const VendorProfile = () => {
  ...
  const getStatusText = (status) => {
    switch (status) {
      case 'pending': return t('vendor.orderPending');
      case 'confirmed': return t('vendor.orderConfirmed');
      case 'in-transit': return t('vendor.orderInTransit');
      case 'delivered': return t('vendor.orderDelivered');
      case 'cancelled': return t('vendor.orderCancelled');
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <nav className="flex space-x-8 overflow-x-auto">
            ...
            >
              {t('vendor.activeOrders')} ({orders.filter(o => o.status !== 'delivered').length})
            </button>
            <button
            ...
            >
              {t('vendor.orderHistory')} ({orders.filter(o => o.status === 'delivered').length})
            </button>
          </nav>
        </div>
          ...
          <>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-medium text-gray-900">{t('vendor.profileDetails')}</h3>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-4">{t('vendor.personalInfo')}</h4>
                ...
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-4">{t('vendor.businessInfo')}</h4>
                  <div className="space-y-3">
                    <div>
            ...
            {showEditProfile && (
              <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                  <div className="mt-3">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">{t('vendor.editProfile')}</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('vendor.name')}</label>
                        <input
                          type="text"
                          value={editForm.name}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('vendor.email')}</label>
                        <input
                          type="email"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('vendor.phone')}</label>
                        <input
                          type="tel"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('vendor.address')}</label>
                        <textarea
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('vendor.businessType')}</label>
                        <select
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-medium text-gray-900">
                {activeTab === 'orders' ? t('vendor.activeOrders') : t('vendor.orderHistory')}
              </h3>
              <Link
                to="/dashboard/vendor"
            ...
            {filteredOrders.map((order) => (
              <div key={order.id} className="bg-white rounded-lg shadow p-6">
                ...
                {/* Order Items */}
                <div className="mb-4">
                  <h5 className="text-sm font-medium text-gray-900 mb-2">{t('vendor.orderItems')}</h5>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">{t('vendor.orderDate')}</label>
                    <p className="text-sm text-gray-900 mt-1">{order.orderDate}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">{t('vendor.expectedDelivery')}</label>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                  <h5 className="text-sm font-medium text-gray-900 mb-2">{t('vendor.supplierContact')}</h5>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium text-gray-700">{t('vendor.phone')}</label>
                      <p className="text-sm text-gray-900 mt-1">{order.supplierPhone}</p>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700">{t('vendor.address')}</label>
                      <p className="text-sm text-gray-900 mt-1">{order.supplierAddress}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex space-x-3">
                  ...
                  {order.status === 'delivered' && (
                    <button
                      onClick={() => { setShowReviewForm(order.id); setReviewForm({ rating: 5, comment: '' }); }}
            ...
            {filteredOrders.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">
                  {activeTab === 'orders' ? t('vendor.noActiveOrders') : t('vendor.noOrderHistory')}
                </h3>
                <p className="text-gray-500">
                  {activeTab === 'orders' ? t('vendor.startShopping') : t('vendor.placeFirstOrder')}
                </p>
              </div>
            )}
          </div>
        ...
        {showReviewForm && (
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
              <div className="mt-3">
                <h3 className="text-lg font-medium text-gray-900 mb-4">{t('vendor.leaveReview')}</h3>
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => {
                      setReviews(r => [...r, { orderId: showReviewForm, supplier: orders.find(o => o.id === showReviewForm)?.supplier, rating: reviewForm.rating, comment: reviewForm.comment }]);
